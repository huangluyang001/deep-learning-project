...
def process_poems_tangshi(file_name):
    poems = []
    '''
...
def run_training(train_set):
    ...
    if train_set == 'poems':
        ...
        poems_vector, word_to_int, vocabularies = process_poems_tangshi('tangshi.txt')
    # 生成batch
...
def gen_poem(begin_word):
    batch_size = 1
    poems_vector, word_int_map, vocabularies = process_poems_tangshi('tangshi.txt')
    input_data = tf.placeholder(tf.int32, [batch_size, None])
    ...
    with tf.Session() as sess:
        ...
        probs_, state_ = sess.run([end_points['prediction'], end_points['last_state']], feed_dict={input_data: x})
        while word != end_token:
            poem += word
            x = np.zeros((1, 1))
            x[0, 0] = word_int_map[word]
            probs_, state_ = sess.run([end_points['prediction'], end_points['last_state']],
                                      feed_dict={input_data: x, end_points['initial_state']: state_})
...
run_training(train_set='poems') # 训练模型
...
tf.reset_default_graph()
print('[INFO] train tang poem...')
run_training(train_set='tangshi') # 训练模型
print('[INFO] write tang poem...')
word_list = ['日','红','山','夜','湖','海','月']
for word in word_list:
    tf.reset_default_graph()
